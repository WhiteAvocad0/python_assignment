...
hospital_list, supplier_list = ["H1","H2","H3","H4"],["S1","S2","S3","S3","S4","S4"]
          ...
          if f.read() == "":
               ...
               with open("ppe.txt","a") as file:
                         for i in range(len(item_code)):
                              file.write(f"{item_code[i]} = {quantity[i]} = {supplier_list[i]}\n")
                         print("Inventory Initiated")
          else:
               login()

##Inventory reset
def inv_reset(fileName,mode):
     with open(fileName,mode) as f:
               for i in range(len(item_code)):
                    f.write(f"{item_code[i]} = {quantity[i]} = {supplier_list[i]}\n")
     print("Inventory reset done")
     admin_menu()
...
def inv_update():
    ...
    with open("transactions.txt","r") as f:
        trans_line,trans_data = f.readlines(),[]
                ...
                match menu_select:
                    ...
                    case 2:
                        if quantity == 0 or change_quantity > quantity:
                             print(f"Insufficient for distribution!")
                             admin_menu()
                        else:
                         new_quantity = quantity - change_quantity
                         ##Print hospital list
                         ...
                         while True:
                              ...
                              try:
                                   ...
                                   to_hospital < len(hospital_list)+1
                              except ValueError:
...
def delete_user(fileName,mode):
     with open(fileName,mode) as f:
          lines = f.readlines()
          ##List check
          uid_data = lines[1].strip().split(",")
          username_data = lines[3].strip().split(",")
          password_data = lines[5].strip().split(",")
          type_data = lines[7].strip().split(",")
          ##Print username list
...
def search_user(fileName,mode):
     ...
     with open(fileName,mode) as f:
          lines = f.readlines()
          ##List check
          uid_data,username_data,password_data,type_data = lines[uid-1].strip().split(","),lines[username-1].strip().split(","),lines[password-1].strip().split(","),lines[type-1].strip().split(",")
     for c,ele in enumerate(username_data,1):
          print(f"{c}. {ele}")
     while True:
